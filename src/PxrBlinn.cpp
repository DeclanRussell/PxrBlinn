...
public:
    ...
    PxrBlinn(RixShadingContext const *sc, RixBxdfFactory *bx,
               RixBXLobeTraits const &lobesWanted,
               RtColorRGB const *blinn, RtFloat const *expo) :
        RixBsdf(sc, bx),
...
#endif
    {
        ...
        for(int i = 0; i < nPts; i++)
        {
            ...
            if (doDiff)
            {
                ...
                if(NdV > k_minfacing)
                {
                    generate(NdV, Nf, m_Tn[i], m_blinn[i],m_expo[i], xi[i],
        ...
        for(int i = 0; i < nPts; i++)
        {
            ...
            if (doDiff)
            {
                ...
                if(NdV > k_minfacing)
                {
                    ...
                    if(NdL > 0.f)
                    {
                        evaluate(NdV, NdL,Nf, m_blinn[i],m_expo[i],Ln[i],m_Vn[i],
...
#endif
    {
        ...
        RtColorRGB const &diff = m_blinn[index];
        RtFloat const &expo = m_expo[index];
        ...
        if(NdV > k_minfacing)
        {
            for(int i=0; i<nsamps; ++i)
            {
                ...
                if(NdL > 0.f)
                {
                    evaluate(NfdV, NdL,Nf, diff,expo,Ln[index],Vn,
                             reflDiffuseWgt[i], FPdf[i], RPdf[i]);
                    lobesEvaluated[i] |= s_reflBlinnLobeTraits;
                }
            }
...
private:
                 ...
                 RtFloat &FPdf, RtFloat &RPdf)
    {
        ...
        // half angle
        float cosTheta = powf(xi.x,1.f/(expo+1.f));
...
//        //PDF
        float blinnPdf = ((expo + 1.f) * powf(cosTheta,expo)) / (2.f * M_PI * 4.f * Ln.Dot(wh));
        ...
        if(cosTheta<=0.f)
            ...
            D = ((expo+2.f)/(M_PI*2.f))*powf(cosTheta,expo);
        ...
        else
        {
            ...
            float a = sqrtf(0.5f*expo+1.f)/tanV;
            if(a<1.6f)
            {
    ...
    PRMAN_INLINE
    void evaluate(RtFloat NdV, RtFloat NdL, RtNormal3 &Nn, const RtColorRGB &color, const RtFloat &expo,
...
private:
    ...
    RtColorRGB const *m_blinn;
    RtFloat const *m_expo;
    RtPoint3 const* m_P;
...
public:
  ...
  private:
    ...
    //----------------------------------------------------------------------------------------------------------------------
    RtColorRGB m_blinnColorDflt;
    //----------------------------------------------------------------------------------------------------------------------
    /// @brief Defualt exponent of our blinn BRDF
    //----------------------------------------------------------------------------------------------------------------------
    RtFloat m_blinnExponentDflt;
    //----------------------------------------------------------------------------------------------------------------------
...
{
    m_blinnColorDflt = RtColorRGB(.5f);
    m_blinnExponentDflt = 1.f;
}
...
PxrBlinnFactory::GetParamTable()
{
    ...
    {
        RixSCParamInfo("blinnColor", k_RixSCColor),
        RixSCParamInfo("exponent", k_RixSCFloat),
...
{
    // Get all input data
    RtColorRGB const * blinn;
    RtFloat const * expo;
    sCtx->EvalParam(k_blinnColor, -1, &blinn, &m_blinnColorDflt, true);
    sCtx->EvalParam(k_blinnExp, -1, &expo, &m_blinnExponentDflt, true);

    RixShadingContext::Allocator pool(sCtx);
    void *mem = pool.AllocForBxdf<PxrBlinn>(1);

    // Must use placement new to set up the vtable properly
    PxrBlinn *eval = new (mem) PxrBlinn(sCtx, this, lobesWanted, blinn,expo);
